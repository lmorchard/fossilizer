...
class ArchiveMain extends HTMLElement {
  connectedCallback() {
  }
}

customElements.define("archive-main", ArchiveMain);
...
class ArchiveActivityListControls extends HTMLElement {
  connectedCallback() {
    const gridButtonEl = this.querySelector("button.toggle-grid");
    if (gridButtonEl) {
      gridButtonEl.addEventListener("click", () => this.toggleGridLayout());
    }
  }

  toggleGridLayout() {
    const forId = this.getAttribute("for");
    const listEl = this.parentElement.querySelector(`#${forId}`);
    listEl.classList.toggle("grid");
  }
}
...
class ArchiveActivityTime extends HTMLElement {
  ...
  update() {
    ...
    this.innerHTML = format(datetime);
  }
}
